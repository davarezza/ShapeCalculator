...
<div class="container">
        <form action="<?php $_SERVER['PHP_SELF'];?>" method="POST">
            <h1 class="mb-3 fs-4">Aplikasi Menghitung Bangun Ruang Kerucut</h1>
            <hr class="border border-danger border-2 opacity-25">
            <h3 class="mb-3 fs-4">Volume Kerucut</h3>
            <div class="mb-3">
        ...
        <?php
            if (isset($_POST['jarivolume']) && isset($_POST['tinggivolume'])) {
                ...
                $hasil = 1/3 * 3.14 * ($jarivolume * $jarivolume) * $tinggivolume;
...
<div class="container mb-4">
        <form action="<?php $_SERVER['PHP_SELF'];?>" method="POST">
            <h1 class="mb-3 fs-4">Aplikasi Menghitung Bangun Ruang Kerucut</h1>
            <hr class="border border-danger border-2 opacity-25">
            <h3 class="mb-3 fs-4">Luas Permukaan Kerucut</h3>
            <div class="mb-3">
                <label for="6" class="form-label">Phi</label>
                <input class="form-control" type="text" value="3,14" disabled readonly>
            </div>
            ...
            <div class="mb-3">
                <label for="garispelukis" class="form-label">Garis Pelukis</label>
                <input type="number" class="form-control" id="garispelukis" name="garispelukis" placeholder="Masukkan Angka..." required>
            </div>
            <button type="submit" class="btn btn-primary">Jumlahkan</button>
        ...
        <?php
            if (isset($_POST['jaripermukaan']) && isset($_POST['garispelukis'])) {
                $jaripermukaan = $_POST['jaripermukaan'];
                $garispelukis = $_POST['garispelukis'];
                $hasil = 3.14 * $jaripermukaan * ($jaripermukaan + $garispelukis);
                echo "<div class='mt-3 fs-5'>Hasilnya adalah $hasil</div>";
            }
        ?>
    </div>
